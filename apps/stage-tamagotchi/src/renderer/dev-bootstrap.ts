...
if (RAW) {
  try {
    ...
    // Mark providers with credentials as "added" so the provider store
    // recognizes them without user interaction.
    if (cfg.providers && Object.keys(cfg.providers).length > 0) {
      const addedRaw = localStorage.getItem('settings/providers/added')
      const added = addedRaw ? JSON.parse(addedRaw) as Record<string, boolean> : {}
